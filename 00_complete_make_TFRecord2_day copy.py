...
#============================================[ 사용자 설정 파라미터 ]==================================================================================
tfrecord_train_filename = '1day50seq38feature_TRAIN_balance .tfrecord'
tfrecord_val_filename = '1day50seq38feature_VAL_balance.tfrecord'
...
def normalize_features(features, normalization_type='none'):
    ...
    if normalization_type == 'min-max':
        ...
        for col in range(22, 38):
            ...
            normalized_features[:, col] = features[:, col] / (max_abs_value +1e-8) 
...
def normalize_sequence(sequence, normalization_type='none'):  # 이게 하나의 샘플에 대해서만 정규화를 수행하는 코드
    """
...
with tf.io.TFRecordWriter(tfrecord_train_path) as train_writer, tf.io.TFRecordWriter(tfrecord_val_path) as val_writer:
    ...
    # 라벨 1의 최대 샘플 수 설정
    max_train_label_1_count = 429910
    max_val_label_1_count = 84885
    ...
    for s in exchange_info['symbols']:
        ...
        try:
            ...
            print(f'IndexError processing {file_path}: {e}. Skipping this file.')
            error_count += 1
            continue
        ...
        for i in range(len(train_features) - sequence_length + 1):
            try:
                ...
                if label == 1 and train_label_1_count >= max_train_label_1_count:
                    continue
        ...
        for i in range(len(val_features) - sequence_length + 1):
            try:
                ...
                if label == 1 and val_label_1_count >= max_val_label_1_count:
                    continue
                example = serialize_example(sequence, label)
