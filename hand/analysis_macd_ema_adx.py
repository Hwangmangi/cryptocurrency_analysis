                    ...
                    for j in range(golden_cross_index + 1, len(macd_diff) - 1):
                        ...
                        if macd_diff[j] >= 0 and macd_diff[j + 1] < 0:
                            ...
                            if return_rate > 31 or return_rate < -31:
                                i = j + 1 
                                break
                            x20 = np.arange(len(data['EMA30'].values[golden_cross_index:dead_cross_index]))
                            mean_gradient_ema20, _, _, _, _ = linregress(x20,data['EMA30'].values[golden_cross_index:dead_cross_index])
                            x30 = np.arange(len(data['EMA40'].values[golden_cross_index:dead_cross_index]))
                            mean_gradient_ema30, _, _, _, _ = linregress(x30,data['EMA40'].values[golden_cross_index:dead_cross_index])
                            x40 = np.arange(len(data['EMA50'].values[golden_cross_index:dead_cross_index]))
                            mean_gradient_ema40, _, _, _, _ = linregress(x40,data['EMA50'].values[golden_cross_index:dead_cross_index])
...
print(f'ema30(+) and {adx_param}에서 돈 딸 확률: {pos_ema20_and_pos_return_rates/(pos_ema20_and_pos_return_rates+pos_ema20_and_neg_return_rates)*100:.2f}%')
print(f'ema40(+) and {adx_param}에서 돈 딸 확률: {pos_ema30_and_pos_return_rates/(pos_ema30_and_pos_return_rates+pos_ema30_and_neg_return_rates)*100:.2f}%')
print(f'ema50(+) and {adx_param}에서 돈 딸 확률: {pos_ema40_and_pos_return_rates/(pos_ema40_and_pos_return_rates+pos_ema40_and_neg_return_rates)*100:.2f}%')
