...
pos_ema20_and_pos_return_rates = []
pos_ema20_and_neg_return_rates = []
pos_ema50_and_pos_return_rates = []
pos_ema50_and_neg_return_rates = []
pos_ema144_and_pos_return_rates = []
pos_ema144_and_neg_return_rates = []
neg_ema20_and_pos_return_rates = []
neg_ema20_and_neg_return_rates = []
neg_ema50_and_pos_return_rates = []
neg_ema50_and_neg_return_rates = []
neg_ema144_and_pos_return_rates = []
neg_ema144_and_neg_return_rates = []
                ...
                if macd_diff[i] <= 0 and macd_diff[i + 1] > 0:
                    ...
                    for j in range(golden_cross_index + 1, len(macd_diff) - 1):
                        ...
                        if macd_diff[j] >= 0 and macd_diff[j + 1] < 0:
                            ...
                            if mean_gradient_ema20 > 0:
                                if return_rate >0:
                                    pos_ema20_and_pos_return_rates.append(return_rate)
                                else:
                                    pos_ema20_and_neg_return_rates.append(return_rate)
                            else:
                                if return_rate >0:
                                    neg_ema20_and_pos_return_rates.append(return_rate)
                                else:
                                    neg_ema20_and_neg_return_rates.append(return_rate)
                            if mean_gradient_ema50 > 0:
                                if return_rate >0:
                                    pos_ema50_and_pos_return_rates.append(return_rate)
                                else:
                                    pos_ema50_and_neg_return_rates.append(return_rate)
                            else:
                                if return_rate >0:
                                    neg_ema50_and_pos_return_rates.append(return_rate)
                                else:
                                    neg_ema50_and_neg_return_rates.append(return_rate)
                            if mean_gradient_ema144 > 0:
                                if return_rate >0:
                                    pos_ema144_and_pos_return_rates.append(return_rate)
                                else:
                                    pos_ema144_and_neg_return_rates.append(return_rate)
                            else:
                                if return_rate >0:
                                    neg_ema144_and_pos_return_rates.append(return_rate)
                                else:
                                    neg_ema144_and_neg_return_rates.append(return_rate)
...
print(f'ema20(+)에서 돈 딸 확률: {len(pos_ema20_and_pos_return_rates)/(len(pos_ema20_and_pos_return_rates)+len(pos_ema20_and_neg_return_rates))*100:.2f}%')
print(f'ema20(-)에서 돈 딸 확률: {len(neg_ema20_and_pos_return_rates)/(len(neg_ema20_and_pos_return_rates)+len(neg_ema20_and_neg_return_rates))*100:.2f}%')
print(f'ema50(+)에서 돈 딸 확률: {len(pos_ema50_and_pos_return_rates)/(len(pos_ema50_and_pos_return_rates)+len(pos_ema50_and_neg_return_rates))*100:.2f}%')
print(f'ema50(-)에서 돈 딸 확률: {len(neg_ema50_and_pos_return_rates)/(len(neg_ema50_and_pos_return_rates)+len(neg_ema50_and_neg_return_rates))*100:.2f}%')
print(f'ema144(+)에서 돈 딸 확률: {len(pos_ema144_and_pos_return_rates)/(len(pos_ema144_and_pos_return_rates)+len(pos_ema144_and_neg_return_rates))*100:.2f}%')
print(f'ema144(-)에서 돈 딸 확률: {len(neg_ema144_and_pos_return_rates)/(len(neg_ema144_and_pos_return_rates)+len(neg_ema144_and_neg_return_rates))*100:.2f}%')
