...
def label_profit_possible(df, lookahead_hours):
    """
    ...
    for i in range(len(df) - lookahead_hours):
        ...
        if any(future_prices > start_price):  # 지정된 시간 내에 더 높은 가격이 있다면 수익 실현 가능
            df['label'].iloc[i] = 1  # 수익 실현 가능 -> 1로 레이블링
        else:
            df['label'].iloc[i] = 0  # 수익 실현 불가능 -> 0으로 레이블링
